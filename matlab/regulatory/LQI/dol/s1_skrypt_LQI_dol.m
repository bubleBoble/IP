...
ampD        = 0.01; % 0.01  % mnożnik zakłóceń
...
%=======================================================================%
E = [1 0 0 0]; % wyj. sterowane = x_wózka, teta
Aa = [A, zeros(4,1); E, zeros(1,1)];
Ba = [B;0];
G  = [zeros(4,1) ; -1];
...
%   LQR 1
%=======================================================================%
Q = diag( [6000e6, 30e6, 400e6, 8e6, 200e6] );
R = 700;
Fa=lqr(Aa, Ba, Q, R);
...
% WOLNO ZMIENNA XW_REF - użyłem do podania zakłócenia
% xw_ref_sim.signals.values(:, 1) = ( ...
%     utils.ustep(t, 0)*track_len/2 + ...
%     utils.ustep(t, 1)*(0.35 - track_len/2) );
...
simout = sim('s1_model_LQI_dol.slx');
x       = simout.state_out;
t       = simout.tout;
u       = simout.u;
d       = simout.d;
